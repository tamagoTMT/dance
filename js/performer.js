document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('../json/performerData.json')
        ...
        .then(data => {
            // 出演者の情報をフィルター
            const performers = data.performers.filter(performer => performerId.split(',').includes(performer.id));
...
function renderPerformers(performers, container) {
    performers.forEach(performer => {
        ...
        performerDiv.innerHTML = `
            ...
            <p><strong>Instagram:</strong> <a href="https://www.instagram.com/${performer.instagram.replace('@', '')}" target="_blank">${performer.instagram}</a></p>
            <p><strong>X (Twitter):</strong> <a href="https://twitter.com/${performer.twitter.replace('@', '')}" target="_blank">${performer.twitter}</a></p>
            <p><strong>あなたにとってダンスとは:</strong> ${performer.whatIsDance}</p>
            <p><strong>振付者の印象やメッセージ:</strong> ${performer.choreographerMessage}</p>
            <p><strong>ご来場のお客様へ一言:</strong> ${performer.messageToAudience}</p>
        `;
